...
public class MySqlSetPanel extends BasePanel
{
    ...
    {
        int labelWidth = 150, labelTextHeight = 25, textWidth = 200;
        ...
        codeCombo = new JComboBox<Object>(new Object[]
        { "utf-8", "gbk" });
        ...
        public void actionPerformed(ActionEvent e)
        {
            ...
            else if (name.equals("保存"))
            {
                ...
                try
                {
                    ...
                    {
                        // HashMap<String, String> sedMap =
                        // reader.getActiveCfg(sections);
                        reader.replaceActiveCfg(sections, conn_name);
                        /*
                         * if(sedMap != null){ reader.putValue(sections,
                         * sedMap.get("conn_name"), "isSelected", "false"); }
                         */
                    }
                    reader.putValue(sections, conn_name, "conn_name", conn_name).putValue(sections, conn_name, "host", ip).putValue(sections, conn_name, "port", port).putValue(sections, conn_name, "userName", username).putValue(sections, conn_name, "password", password).putValue(sections, conn_name, "db_name", dnname).putValue(sections, conn_name, "isSelected", isSelected).putValue(sections, conn_name, "encode", encode).putValue("use_db_type", "db", "type", sections).save();
